...
function drink(beerName) {
  ...
  // console.log(beers.hasOwnProperty(beerName) instanceof Array);
  if(beers.hasOwnProperty(beerName) === true) {
    if(typeof beers[beerName] === "object") {
      var moreDrinks = beers[beerName].join(" and ");
      return "This " + beerName + " is " + moreDrinks + ".";
    }
    else {
...
function listLivingOrgClass() {
  ...
  for(var i = 0; i < livingOrganismClassification.length; i++) {
    if(livingOrganismClassification[i] === true) {
      return '<li> ' + livingOrganismClassification + ' </li>';
    }
    else {
      return htmlUL === '</ul>';
    }
...
function favoritePlanet(currentPlanet) {
  ...
  } else if(planets.indexOf(currentPlanet) >= 0) {
    return ("I'm from " + currentPlanet + ", but I wish I could go to " + planets + ".");
  }
}
...
this.gender = gender;
  ...
  Person.prototype.spendMoney = function(spend) {
    return (this.money -= spend);
  };

  Person.prototype.earnMoney = function(earn) {
    return (this.money += earn);
  };
}
...
function Shape(sides) {
  ...
  if(this.sides < 3 || typeof sides !== "number") {
    this.sides['sides'] = null;
    return this.sides;
...
var nemo = new Animal('Mon', 'female');
console.log(george);
console.log(nemo);
